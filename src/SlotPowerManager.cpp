...
void SlotPowerManager::update(const ManagedObjectType& sensorConfigurations)
{
    ...
    lg2::debug("Found {SIZE} device configs", "SIZE", deviceConfigs.size());
    ...
    for (auto& deviceInfo : deviceConfigs)
    {
        ...
        {
            lg2::error("No slot found for sensor location code {LOC}", "LOC",
        ...
        {
            lg2::debug("Initial power state of {SLOT} is {STATE}", "SLOT",
                       slotName, "STATE", powerState);
        ...
        else
        {
            ...
            // this function call.
            if (std::ranges::find(newSlots, slotName) != newSlots.end())
            {
                deviceInfo.state = powerState;
    ...
    std::vector<DeviceInfo>& deviceConfigs)
{
    for (const auto& [objectPath, sensorData] : sensorConfigurations)
    {
        for (const auto& [type, config] : sensorData)
        {
            ...
            {
                lg2::error(
                    "Invalid PowerStateOwner type: {OWNER} in entity-manager "
                    "config",
                    "OWNER", owner);
                continue;
            }
            ...
            {
                lg2::error("Missing LocationCode entry on {PATH}", "PATH",
                           objectPath.str);
                continue;
            }
            ...
            {
                lg2::error("Missing Bus or Address for {PATH}", "PATH",
                           objectPath.str);
                continue;
            }
            ...
            {
                lg2::error("Invalid Bus or Address for {PATH}", "PATH",
                           objectPath.str);
                continue;
            }
            ...
            lg2::debug("DeviceInfo: Name: {NAME}, Type: {TYPE}", "NAME",
...
void SlotPowerManager::getSlots(std::map<std::string, std::string>& slots)
{
    ...
    for (const auto& [path, objDict] : subtree)
    {
        ...
        {
            lg2::error("Failed getting location code from {PATH}: {EX}", "PATH",
                       path, "EX", ex);
        }
    }
}
...
std::string SlotPowerManager::getPowerState(const std::string& path)
{
    ...
    auto entryIt =
        std::ranges::find_if(subtree, [path](const auto& subtreeEntry) {
...
void SlotPowerManager::powerStateChanged(sdbusplus::message_t& msg)
{
    ...
    lg2::debug("Power state of {PATH} changed to {STATE}", "PATH", objectPath,
    ...
    std::ranges::for_each(deviceConfigs, [powerState](auto& config) {
        config.state = powerState;
    });
...
bool SlotPowerManager::isDeviceOff(uint64_t bus, uint64_t address) const
{
    ...
    {
        auto deviceIt =
            std::ranges::find_if(devices, [bus, address](const auto& device) {
